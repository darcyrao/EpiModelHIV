...
#' @title Make sure specified proportions sum to 1
#'
#' @description Checks that specified proportions sum to 1 to account for rounding
#'              in calculation of the input parameters. If the sum of a vector of 
#'              values is within 0.001 of 1, this function adds or subtracts from 
#'               each of the set of values such that the total sums to 1. If the sum 
#'               of the values is more than 0.001 greater or smaller than 1, it 
#'               returns an error.
#'
#' @param values Values of the input vector.
#'
#' @export
#'
sumto1 <- function(values) {
  
  if (abs(1 - sum(values)) > 0.001) {
    stop("sum of argument values must be within 0.001 of 1")
  }
  ...
  vector.length <- length(values)
  result <- rep(NA, vector.length)
  error <- 1 - sum(values)
  if (error == 0) {
    result <- values
  } else {
    adjustment <- error / vector.length
