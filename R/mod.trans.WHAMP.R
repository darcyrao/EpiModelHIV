...
#' acute stage infection excess risk, condom use, circumcision, and the CCR5 
#' genetic allele. Given these transmission probabilities, transmission is 
#' stochastically simulated by drawing from a binomial distribution for each 
#' act conditional on the per-act probability.
#'
#' @return
#' For each new infection, the disease status, infection time, and related
#' HIV attributes are updated for the newly-infected node. Summary statistics for
#' disease incidence overall and by race/ethnicity, age, and region are calculated 
#' and stored on \code{dat$epi}.
#'
#' @keywords module msm
#'
#' @export
#'
trans_msm_whamp <- function(dat, at) {
  ...
  dat$epi$incid.B[at] <- sum(race..wa[infected] == "B", na.rm = TRUE)
  dat$epi$incid.H[at] <- sum(race..wa[infected] == "H", na.rm = TRUE)
  dat$epi$incid.O[at] <- sum(race..wa[infected] == "O", na.rm = TRUE)
  dat$epi$incid.KC[at] <- sum(region[infected] == "KC", na.rm = TRUE)
  dat$epi$incid.OW[at] <- sum(region[infected] == "OW", na.rm = TRUE)
  dat$epi$incid.EW[at] <- sum(region[infected] == "EW", na.rm = TRUE)
  dat$epi$incid.prep0[at] <- sum(prepStat[infected] == 0, na.rm = TRUE)
  dat$epi$incid.prep1[at] <- sum(prepStat[infected] == 1, na.rm = TRUE)
  # dat$epi$incid.18to24[at] <- sum(age[infected] < 25, na.rm = TRUE)
  # dat$epi$incid.25to29[at] <- sum(age[infected] >= 25 & age[infected] <30, na.rm = TRUE)
  # dat$epi$incid.30to34[at] <- sum(age[infected] >= 30 & age[infected] <35, na.rm = TRUE)
  # dat$epi$incid.35to39[at] <- sum(age[infected] >= 35 & age[infected] <40, na.rm = TRUE)
  # dat$epi$incid.40to44[at] <- sum(age[infected] >= 40 & age[infected] <45, na.rm = TRUE)
  # dat$epi$incid.45to49[at] <- sum(age[infected] >= 45 & age[infected] <50, na.rm = TRUE)
  # dat$epi$incid.50to54[at] <- sum(age[infected] >= 50 & age[infected] <55, na.rm = TRUE)
  # dat$epi$incid.55to59[at] <- sum(age[infected] >= 55 & age[infected] <60, na.rm = TRUE)
