...
#'
#' @description Module function for updating HIV viral load for the WHAMP model.
#'
...
#' chronic stage infection. VL again rises during untreated AIDS stage disease 
#' until the point of death.
#'
#' For persons who have ever initated treatment, VL changes depending on current ART
#' use, time since ART (re)initiation, or time since ART cessation. Current use is 
#' associated with a reduction in VL, with the rates of decline and nadirs dependent 
#' on partial or full suppression levels. Treatment cessation is associated with an 
#' equal rate of increase in VL up to set point. For individuals who have progressed to
#' AIDS, VL rises up to set point at the same rate as individuals in chronic stage infection
#' and then rises beyond that at the rate that results in VL of 7 log10 after 2 years. 
#' Because HIV-related mortality is modeled as a constant increased hazard of death rather
...
vl_msm_whamp <- function(dat, at) {
  ...
  vldo <- dat$param$vl.aids.onset.int
  ...
  new.vl <- (inf.time.bp.tn <= vlard) * (vlap * inf.time.bp.tn / vlard) +
            ...
            (inf.time.bp.tn > vldo) * pmin((vlsp + (inf.time.bp.tn - vldo) * vlds), vlaidsp)
  vl[target] <- new.vl

  # 2. full suppressors on tx
  target <- which(tx.status == 1 & tt.traj %in% c(2,4))
  current.vl <- vl[target]
  new.vl <- pmax(current.vl - full.supp.down.slope, vl.full.supp)
  vl[target] <- new.vl

  # 3. partial suppressors on tx
  target <- which(tx.status == 1 & tt.traj %in% c(1,3))
  current.vl <- vl[target]
  new.vl <- pmax(current.vl - part.supp.down.slope, vl.part.supp)
  vl[target] <- new.vl

  # 4. full suppressors, discontinued, not yet in AIDS phase
  target <- which(tx.status == 0 & cum.time.on.tx > 0 & tt.traj %in% c(2,4) & stage != 4)
  current.vl <- vl[target]
  new.vl <- pmin(current.vl + full.supp.up.slope, vlsp)
  vl[target] <- new.vl
  
  # 5. partial suppressors, discontinued, not yet in AIDS phase
  target <- which(tx.status == 0 & cum.time.on.tx > 0 & tt.traj %in% c(1,3) & stage != 4)
  current.vl <- vl[target]
  new.vl <- pmin(current.vl + part.supp.up.slope, vlsp)
  vl[target] <- new.vl

  # 6. full suppressors, discontinued, in AIDS phase, current VL < set point
  target <- which(tx.status == 0 & cum.time.on.tx > 0 & 
                    tt.traj %in% c(2,4) & stage == 4 &
                    vl < vlsp)
  current.vl <- vl[target]
  new.vl <- pmin(current.vl + full.supp.up.slope, vlsp)
  vl[target] <- new.vl
  
  # 7. partial suppressors, discontinued, in AIDS phase, current VL < set point
  target <- which(tx.status == 0 & cum.time.on.tx > 0 & 
                    tt.traj %in% c(1,3) & stage == 4 &
                    vl < vlsp)
  current.vl <- vl[target]
  new.vl <- pmin(current.vl + part.supp.up.slope, vlsp)
  vl[target] <- new.vl
  
  # 8. All who have discontinued, in AIDS phase, current VL >= set point
  target <- which(tx.status == 0 & cum.time.on.tx > 0 & 
                    stage == 4 &
                    vl >= vlsp)
  current.vl <- vl[target]
  new.vl <- pmin(current.vl + vlds, vlaidsp)
  vl[target] <- new.vl
